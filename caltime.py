...
def record_time(stage_name, start_time):
    end_time = time.time()
    timing_results[stage_name] = end_time - start_time
    print(f"{stage_name}: {timing_results[stage_name]:.4f}秒")
    return end_time


total_start_time = time.time()
print("="*60)
print("开始PNCS算法详细时间测量")
print("="*60)
if file_id==1:
    file_name = "data1.csv"
elif file_id==2:
...
stage1_end = record_time("阶段1_数据读取和预处理", stage1_start)
...
stage2_end = record_time("阶段2_聚类中心计算", stage2_start)
...
stage3_end = record_time("阶段3_投影矩阵和加噪", stage3_start)
...
stage5_end = record_time("阶段5_聚类中心距离排序", stage5_start)
...
stage6_end = record_time("阶段6_评分与聚类分配", stage6_start)
...
stage7_end = record_time("阶段7_候选集计算", stage7_start)
...
timing_results['总执行时间'] = total_end_time - total_start_time

print("\n" + "="*60)
print("PNCS-DIFEPS各阶段时间统计")
print("="*60)
